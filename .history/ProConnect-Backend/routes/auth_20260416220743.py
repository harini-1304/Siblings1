...
def forgot_password():
    ...
    try:
        ...
        try:
            ...
            print(f"📧 Reset link (for development): {reset_link}")
            # In development, show the reset link for testing
            if os.getenv('FLASK_ENV') == 'development':
                return jsonify({
                    'message': 'Password reset link generated (email service failed in development)',
                    'resetLink': reset_link  # For development testing only
                }), 200
            # In production, don't reveal email issues
            return jsonify({
                'message': 'If the email exists, a password reset link will be sent shortly.'
            }), 200
