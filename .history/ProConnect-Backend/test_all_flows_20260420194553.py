...
def test_student_record_access():
    ...
    try:
        ...
        # Test 3: Mixed case match (use a real email with different casing)
        # Create mixed case version of the actual college email
        test_email_mixed = 'CamelCase' + college_mail[9:] if college_mail.startswith('24z260') else college_mail.upper()[:5] + college_mail[5:].lower()
        found = students.find_one({
            'basic_info.roll_number': roll_number,
            '$or': [
